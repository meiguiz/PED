...
class Inferencer(object):
    ...
    def __init__(self, cfg_path = os.path.join('configs', 'inference.yaml'), with_info = False, **kwargs):
        ...
        self.depth_gtmin, self.depth_gtmax = self.builder.get_inference_depth_min_max()

        self.depth_norm = self.builder.get_inference_depth_norm()
        self.depth_gtnorm = self.builder.get_inference_depth_norm()


    def inference(self, rgb, depth, depth_gt, target_size = (1280, 720), depth_coefficient = 10.0, inpainting = True):
        """
        Inference.
        ...
        depth_gt = np.where(depth_gt < self.depth_gtmin, 0, depth_gt)
        depth_gt = np.where(depth_gt > self.depth_gtmax, 0, depth_gt)
        depth_gt[np.isnan(depth)] = 0
        depth_gtavailable = depth_gt[depth > 0]
        depth_gtmu = depth_gtavailable.mean() if depth_gtavailable.shape[0] != 0 else 0
        depth_gtstd = depth_gtavailable.std() if depth_gtavailable.shape[0] != 0 else 1
        depth_gt = np.where(depth_gt < depth_gtmu - depth_coefficient * depth_gtstd, 0, depth_gt)
        depth_gt = np.where(depth_gt > depth_gtmu + depth_coefficient * depth_gtstd, 0, depth_gt)
        if inpainting:
            maskgt = np.where(depth_gt > 0)
            if maskgt[0].shape[0] != 0:
                interp = NearestNDInterpolator(np.transpose(maskgt), depth_gt[maskgt])
                depth_gt = interp(*np.indices(depth_gt.shape))
        depth_gt = depth_gt / self.depth_gtnorm
        depth_gtmin = depth_gt.min() - 0.5 * depth_gt.std() - 1e-6
        depth_gtmax = depth_gt.max() + 0.5 * depth_gt.std() + 1e-6
        depth_gt = (depth_gt - depth_gtmin) / (depth_gtmax - depth_gtmin)
        ...
        depth_gt = torch.FloatTensor(depth_gt).to(self.device).unsqueeze(0)
        ...
        depth_gtori = cv2.resize(depth_gtori, target_size, interpolation = cv2.INTER_NEAREST)

        return depth_res, depth_ori, depth_gtori
